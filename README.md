...
Learn to build Go clients with progressive complexity in `clients/go/workshop/`:

1. **Session 1: Basic HTTP Client** - HTTP fundamentals without caching
2. **Session 2: Redis Cache Client** - Exact-match caching with Redis (~18,000x speedup!)
3. **Session 3: LangCache Client** - Semantic caching with benchmarking
...
See [QUICKSTART.md](QUICKSTART.md) or [clients/go/workshop/README.md](clients/go/workshop/README.md) for details.

## Go Client Libraries

Production-ready Go client libraries are available in `clients/go/`:

1. **Basic HTTP Client** (`basic/`) - Simple client without caching
2. **Redis Cache Client** (`redis_cache/`) - Client with exact-match Redis caching
3. **LangCache Client** (`langcache/`) - Client with semantic caching via Redis LangCache API

See [clients/go/README.md](clients/go/README.md) for library documentation.
